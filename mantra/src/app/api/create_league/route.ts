...
export async function POST(request: Request) {
  ...
  const { name, league_id, description, teams, price, events_included } = league_details;
  try {
    const response = await createLeague(name, league_id, description, teams, price, events_included);
